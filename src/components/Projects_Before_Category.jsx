...
import { X, ArrowRight, Filter, Calendar, User, Tag } from 'lucide-react';
...
const ProjectDetail = ({ project, onClose }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      if (distance < 80) {
        gsap.to(btn, { x: distanceX * 0.3, y: distanceY * 0.3, duration: 0.4 });
      } else {
      ...
      >
        ...
        <div className="relative z-10 min-h-screen flex items-center justify-center p-6 md:p-12">
          <motion.div
            ...
            className="w-full max-w-5xl bg-black/60 backdrop-blur-xl border border-white/10 rounded-3xl p-8 md:p-12 shadow-2xl"
          >
            {/* Close Button */}
            <button
              ref={closeBtnRef}
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                <span className="px-4 py-2 bg-indigo-500/20 backdrop-blur-md rounded-full text-xs font-space font-bold uppercase tracking-widest text-indigo-400 border border-indigo-500/30">
                  {project.category}
                </span>
              </div>

              <h1 className="text-5xl md:text-7xl font-space font-black text-white mb-6 tracking-tight leading-none">
                {project.title}
              </h1>
              ...
              <div className="flex flex-wrap gap-6 text-white/60">
                <div className="flex items-center gap-2">
                  <User size={16} />
                  <span className="text-sm font-space uppercase tracking-wide">
                ...
                <div className="flex items-center gap-2">
                  <Calendar size={16} />
                  <span className="text-sm font-space uppercase tracking-wide">
                    <span className="text-white/40">Année:</span> {project.year}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <Tag size={16} />
                  <span className="text-sm font-space uppercase tracking-wide">
                    <span className="text-white/40">Type:</span> {project.category}
                  </span>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="mb-10"
            >
              <h2 className="text-xl font-space font-bold text-white mb-4 uppercase tracking-wide">Description</h2>
              <p className="text-lg font-inter text-white/80 leading-relaxed max-w-3xl">
                {project.description}
              </p>
            </motion.div>
            ...
            >
              <h2 className="text-xl font-space font-bold text-white mb-6 uppercase tracking-wide">Aperçus du Projet</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {project.galleryImages.map((img, idx) => (
                  <motion.div
                    ...
                    className="relative aspect-[4/5] rounded-xl overflow-hidden cursor-pointer group border border-white/10"
